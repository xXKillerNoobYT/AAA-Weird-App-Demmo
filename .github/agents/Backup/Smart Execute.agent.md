...
tools:
  ['vscode', 'execute', 'read', 'edit', 'search', 'web', 'mcp_docker/*', 'agent', 'pylance-mcp-server/*', 'memory', 'mermaidchart.vscode-mermaid-chart/get_syntax_docs', 'mermaidchart.vscode-mermaid-chart/mermaid-diagram-validator', 'mermaidchart.vscode-mermaid-chart/mermaid-diagram-preview', 'todo']
...
- `/memories/dev/smart-plan/` (read-only for plan context)
- `/memories/system/` (read-only)
- `/memories/autogen/` (read-only)

**See:** `AI Files/MEMORY_ORGANIZATION_SYSTEM.md`

**Write immediately:** Log execution steps, tool usage, errors, solutions to memory as they happen.
...
# Total: ~128 tools via wildcards (generous, execution-focused)
  ...
  # .NET: ~10-15 tools via dotnet-mcp-server/* (if available)