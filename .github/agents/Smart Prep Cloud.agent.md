...
handoffs:
  - label: Hand off to Cloud Agent
    agent: Cloud Agent
    prompt: Execute via @cloud mention or /delegate command with prepared issue and TODOs
    send: true
---
...
**Your namespace:** `/memories/dev/smart-prep-cloud/`

**Allowed paths:**
- `/memories/dev/smart-prep-cloud/` (read/write)
- `/memories/dev/shared/` (read/write)
- `/memories/system/` (read-only)

**Store:** Cloud confidence calculations, environment validations, issue templates.
...
**Key Principle:** Cloud agents lack IDE context and MCP discovery, so all instructions must be embedded in the issue with exact commands, environment setup, and validation criteria.
  ...
  # Limited to ~25 tools (cloud prep is focused, not execution)
  # Core read-only: file_search, read_file, semantic_search, memory
  # MCP Docker: cloud prep specific tools (7 tools)
  # GitHub: issue/PR operations (3 tools)